...
use crate::{queue::VirtQueue, Error, Hal, Result, VirtIOHeader, PAGE_SIZE};

/// TODO: Add docs
pub struct VirtIOSound<'a, H: Hal> {
    header: &'static mut VirtIOHeader,

    control_queue: VirtQueue<'a, H>,
    event_queue: VirtQueue<'a, H>,
    tx_queue: VirtQueue<'a, H>,
    rx_queue: VirtQueue<'a, H>,
...
impl<H: Hal> VirtIOSound<'_, H> {
    /// Craete a new VirtIO-Sound driver.
    pub fn new(header: &'static mut VirtIOHeader) -> Result<Self> {
        let mut negotiated_features = SoundFeatures::empty();
        header.begin_init(|features| {
            let device_features = SoundFeatures::from_bits_truncate(features);
            negotiated_features = SUPPORTED_FEATURES & device_features;
        ...
        let control_queue = VirtQueue::new(header, CONTROL_QUEUE_IDX, QUEUE_SIZE)?;
        ...
        let event_queue = VirtQueue::new(header, CONTROL_QUEUE_IDX, QUEUE_SIZE)?;
        let tx_queue = VirtQueue::new(header, CONTROL_QUEUE_IDX, QUEUE_SIZE)?;
        let rx_queue = VirtQueue::new(header, CONTROL_QUEUE_IDX, QUEUE_SIZE)?;

        // read configuration space
        let config = unsafe { &mut *(header.config_space() as *mut VirtIOSoundConfig) };
        let jacks = config.jacks.read();
        let streams = config.streams.read();
        let chmaps = config.chmaps.read();
        ...
        let queue_buf_send = vec![0; PAGE_SIZE].into_boxed_slice();
        let queue_buf_recv = vec![0; PAGE_SIZE].into_boxed_slice();

        let output_rsp = vec![0; PAGE_SIZE].into_boxed_slice();

        let event_buf = vec![0; PAGE_SIZE].into_boxed_slice();
    ...
    fn request<Req: AsBytes, Rsp: FromBytes>(&mut self, req: Req) -> Result<Rsp> {
        ...
        self.control_queue
            .add(&[&self.queue_buf_send], &mut [&mut self.queue_buf_recv])?;
        self.header.notify(CONTROL_QUEUE_IDX as u32);
    ...
    pub fn jack_remap(&mut self, jack_id: u32, association: u32, sequence: u32) -> Result {
        ...
        let jack_features = JackFeatures {
            bits: self
                .jack_infos
                .as_ref()
                .unwrap()
    ...
    pub fn pcm_xfer(&mut self, stream_id: u32, frames: &[u8]) -> Result {
        ...
        for i in 2..xfer_times {
            self.header.notify(TX_QUEUE_IDX as u32);
            ...
            if turn1 {
                ...
                buf2[U32_SIZE..U32_SIZE + end_byte - start_byte]
                    .copy_from_slice(&frames[start_byte..end_byte]);
                self.tx_queue.add(&[&buf2], &mut [&mut outputs]).unwrap();
            }
        }
    ...
    pub fn pcm_xfer_nb(&mut self, stream_id: u32, frames: &[u8]) -> Result<u16> {
        ...
        let token = self.tx_queue.add(&[&buf], &mut [&mut self.output_rsp])?;
        self.header.notify(TX_QUEUE_IDX as u32);
    ...
    pub fn pcm_xfer_ok(&mut self, token: u16) -> Result {
        ...
        if let Err(_) = self.tx_queue.pop_used() {
            Err(Error::IoError)
        } else {
    ...
    pub fn rates_supported(&mut self, stream_id: u32) -> Result<PcmRate> {
        ...
        Ok(PcmRate { bits: self.pcm_infos.as_ref().unwrap()[stream_id as usize]
            .rate })
    }
    ...
    pub fn formats_supported(&mut self, stream_id: u32) -> Result<PcmFormats> {
        ...
        Ok(PcmFormats { bits: self.pcm_infos.as_ref().unwrap()[stream_id as usize]
            .formats })
    }
    ...
    pub fn features_supported(&mut self, stream_id: u32) -> Result<PcmFeatures> {
        ...
        let pcm_info = &self.pcm_infos.as_ref().unwrap()[stream_id as usize];
        Ok(PcmFeatures { bits: pcm_info.features })
    }
...
const CONTROL_QUEUE_IDX: usize = 0;
const EVENT_QUEUE_IDX: usize = 1;
const TX_QUEUE_IDX: usize = 2;
const RX_QUEUE_IDX: usize = 3;
...
//         let jack_features = JackFeatures::from(JackFeatures {
//             bits: self.features,
//         });
...
impl Default for PcmParameters {
    fn default() -> Self {
        Self {
            ...
            features: PcmFeatures { bits: 0 },
            channels: Default::default(),
            format: PcmFormats { bits: 0 },
            rate: PcmRate { bits: 0 },
        }
    }
}
